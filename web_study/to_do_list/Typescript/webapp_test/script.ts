...
const inputSec:HTMLElement | null = document.querySelector("#inputText");
const contentWrapper:HTMLElement | null = document.querySelector("#contentWrapper");

const ipAdress: string = `nathankys.mynetgear.com:7700`;

function ajaJPromise(method: string ,url?: string ,data?: any /* any will be fixed.. */ ){
...
function setAction(newContent){
    newContent.childNodes[0].onclick = ()=>{    // toggle clicking content
    ...
    newContent.childNodes[1].onclick = ()=>{    // fix
    ...
    newContent.childNodes[2].onclick = ()=>{    // delete
        const indexNum = Array.from(newContent.parentNode.childNodes).indexOf(newContent); 
