...
function freshList(){
    ajaJPromise(`GET`,`http://${ipAdress}/api/lists`)//
        ...
        .catch(console.log);
    // xhr.open(`GET`,`http://${ipAdress}/api/lists`);
    // xhr.onload = ()=>{
    //     const data = JSON.parse(xhr.response);
    //     for(let i=0; i<data.length; i++){
    //         const newContent = createList(data[i].content);
...
function addList(){
    ...
    // xhr.open(`POST`,`http://${ipAdress}/api/lists`);
    // xhr.onload = ()=>{
    //     if(xhr.status === 400){
    //         alert(xhr.response);
    //     }
    ...
    //         const newContent = createList(data.content);
    //         contentWrapper.append(newContent);
...
function setAction(newContent){
    ...
    newContent.childNodes[1].onclick = ()=>{    // fix
        if(newContent.childNodes[1].innerHTML === `Edit`){
            ...
            //     xhr.open(`PUT`,`http://${ipAdress}/api/lists/${indexNum}`);
            //     xhr.onload = ()=>{
            //         if(xhr.status === 400){
            //             alert(xhr.response);
            //             newContent.childNodes[0].focus();
    ...
    newContent.childNodes[2].onclick = ()=>{    // delete
        ...
        // xhr.open(`DELETE`,`http://${ipAdress}/api/lists/${indexNum}`);
        // xhr.onload = ()=>{
        //     if(xhr.status === 404){
        //         alert(xhr.response);
        //         return;
